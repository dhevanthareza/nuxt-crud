<template>
  <v-app>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">Province</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
...
export default {
  ...
  methods: {
    ...
    async delete(item) {
      await request.delete(`/provinces/${item.id}`)
      this.getProvincesList()
    },
    async saveItem() {
      ...
      try {
        if ('id' in this.detail) {
          await request.put(`/provinces/${this.detail.id}`, {
    ...
    async getProvincesList() {
      ...
      const data = await request.get(
        `/provinces?filter=${JSON.stringify(this.filter)}`,
      )
      const totalItems = await request.get('/provinces/count')
